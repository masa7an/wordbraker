...
### 未実施（優先度順）
- [ ] **描画処理の軽量化**
  - [ ] **1. テキスト描画のキャッシュ化**（スコア、タイマー、お題など、値が変わった時だけ再レンダリング）
  - [ ] **2. ブロックの描画をSurface blitに変更**（draw.rect → blit、3回/フレーム）
  - [ ] **3. パドル・扉の描画をSurface blitに変更**（draw.rect → blit）
...
### 目標値
- 平均60FPS（フレーム時間 16.7ms以下）
- フレーム落ちなし（最大フレーム時間 33ms以下）
