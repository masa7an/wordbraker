...
## フェーズ12: ボール発射角度の調整機能 ✅ 完了

### 目的
クリック時のマウス位置に応じてボールの発射角度を調整し、プレイヤーが最初の発射である程度方向を狙えるようにする。

### 実装内容
- [x] マウス位置に応じた発射角度の計算
  - [x] マウス位置が左側の場合：左向きの角度範囲でランダム発射
  - [x] マウス位置が右側の場合：右向きの角度範囲でランダム発射
  - [x] 中央付近の場合は従来通りランダム
- [x] `ball.launch()` メソッドの拡張（マウス位置を受け取る）
- [x] クリック時のマウス位置取得と角度計算
- [x] キーボード左右キーによる角度調整（ゲームパッドの代わり）

### 仕様
- 画面中央を基準に、マウス位置が左側か右側かを判定
...
## フェーズ13: パドル反射の改善（Arkanoid方式） ✅ 完了

### 目的
パドル反射をヒット位置のみで角度を決定する方式（Arkanoid方式）に変更し、プレイヤーが直感的にボールをコントロールできるようにする。

### 実装内容
- [x] ヒット位置による角度変化（Arkanoid方式）の実装
  - [x] パドルのどこに当たったかだけで反射角度を決定
  - [x] 最大45度まで曲げる（パドルの端で最大角度）
  - [x] パドルの移動方向による角度追加を削除
- [x] 見た目の傾きは維持（パドルの描画は変更しない）

### 仕様
- パドル中央に当たれば、ボールは垂直に近く跳ね返る
- パドルの端に当たるほど、横方向へ強く跳ね返る（最大45度）
- 「右に飛ばしたいなら右端で受ける」という直感的な操作が可能
- タイミングを合わせる必要がなく、位置合わせだけで済む
